...
User picks a CSV; `load_file_as_dicts()` reads it with `csv.DictReader` and every field is looked up with `.get(col, default)`, so extra/missing columns are tolerated. **`plan_format_example.csv` (repo root) is the canonical schema** — copy it and edit. Columns: `run`, `label`, `mode` (`treatment` | `qa`, default `treatment`), `start_x/start_y/start_r`, the 14 run-form values (`num_alpides`, `num_events`, `strobe`, `ithr`, `energy`, `MU`, `current`, `Exposure time (ms)`, `Beam delay (ms)`, `Loops`, `Trigger Freq. (Hz)`, `X step (mm)`, `Y step (mm)`, `R step (degree)`), then `qa_pos_x/qa_pos_y/qa_pos_r` + `vel_x/vel_y/vel_r` (used only when `mode=qa`; leave blank for treatment rows). Working plan files live in the gitignored `plan/`.
