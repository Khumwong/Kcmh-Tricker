...
- Treatment per-loop gate-open window = `Beam delay (ms)` + `Exposure time (ms)` + `Beam off delay (ms)` (`_window_ms_per_loop()`). `Beam off delay` keeps the FPGA trigger running past the exposure so ALPIDE can confirm the beam is gone *before* `\xEF` closes the gate and the phantom steps — measured beam turn-on lag at KCMH is ~300 ms and the trailing edge is a hard cut with no FPGA-side delay, so a lingering beam during the stage move was otherwise invisible. GUI-side window timing only — it is NOT sent to the FPGA (only `Beam delay` becomes the `alpide_delay` byte, ≤255). QA mode ignores all three (one continuous window).
